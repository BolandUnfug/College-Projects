...
public abstract class TreeUtilities{
	...
	public static int getMax(TNode nd){
		return nd.getRight() != null ? getMax(nd.getRight()) : nd.getData();
	}

	public static int getTotal(TNode nd, int counter){ // to be added later for fun, counts the number of items  in the tree
		int temp1 = 0;
		int temp2 = 0;
		if(nd.getLeft() != null){
	...
	public static int getNodeHeight(TNode nd, int target,int counter){
	...
	public static int getTreeHeight(TNode nd, int counter){
		if(nd != null){
			counter++;
	...
	public static int getNextHighest(TNode nd, int target){
    ...
    public static void printTreeHelper(TNode nd){
	if (nd != null){
    ...
    //This function prints all of the data an BinomialTree in order
    //Starting with the lowest and going to the highest
    public static void printTree(BinomialTree tree){
	printTreeHelper(tree.getRoot());
	System.out.println("");
    }

    //This function creates a BinomialTree filled with randomly chosen data
    //num is the number of nodes in the random search tree
    //max_val is the upper limit on the randomly chosen integer data
    public static BinomialTree makeRandomTree(int num, int max_val){
