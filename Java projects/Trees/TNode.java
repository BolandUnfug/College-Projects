//This is a binary search tree that stores integers

public class TNode{
    public TNode left;
    public TNode right;
    public TNode parent;
    public int data;

    public void insert(int new_data){
	if( new_data >= data){
	    //Insert larger values on the right
	    if( right == null){
		right = new TNode(new_data);
		right.setParent(this);
	    }
	    else{
		right.insert(new_data);
	    }
	}

	if( new_data < data){
	    //Insert smaller values on the left
	    if( left == null){
		left = new TNode(new_data);
		left.setParent(this);
	    }
	    else{
		left.insert(new_data);
	    }
	}
    }

    public boolean search(int target){
	if( data == target ){
	    return true;
	}
	if( target > data ){
	    //Find larger values on the right
	    if( right != null){
		return right.search(target);
	    }
	}

	if( target < data){
	    //Find smaller values on the left
	    if(left != null){
		return left.search(target);
	    }
	}
	
	return false;
    }

	public TNode nodeSearch(int target){
		if( data == target ){
			return this;
		}
		if( target > data ){
			//Find larger values on the right
			if( right != null){
			return right.nodeSearch(target);
			}
		}
	
		if( target < data){
			//Find smaller values on the left
			if(left != null){
			return left.nodeSearch(target);
			}
		}
		
		return null;
		}

	

    public int getData(){
	return data;
    }

    public void setData(int d){
	data = d;
    }

    public TNode getLeft(){
	return left;
    }

    public void setLeft(TNode nd){
	left = nd;
    }

    public TNode getRight(){
	return right;
    }

    public void setRight(TNode nd){
	right = nd;
    }

    public TNode getParent(){
	return parent;
    }

    public void setParent(TNode nd){
	parent = nd;
    }

    public boolean isLeaf(){
	if ( (left == null) && (right == null) ){
	    return true;
	}
	else {
	    return false;
	}
    }

    //Constructor
    public TNode(int the_data){
	data = the_data;
    }
}
